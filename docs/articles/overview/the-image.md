...
Se isso não estiver claro, talvez devesse dedicar um tempinho a mais no entendimento tanto do artigo quanto dos _links_ relacionados nele, e que falam sobre as outras definições de arquitetura.

## Simplificação da imagem

Nós procuramos simplificar essa imagem para que seja mais prático olhar para ela e perceber esses detalhes. Então partimos da imagem original, mas para isso removemos alguns detalhes dela e chegamos a essa imagem como sendo o ponto mais importante do artigo original:

![The Clean Architecture resume image](../../images/TheCleanArchitectureOriginalSmall.png)

Essa imagem anterior talvez seja o resumo da _Arquitetura Limpa_ conforme a maioria se baseia para implementar. Mas por ironia ela acaba referenciando quase que diretamente uma das definições que se conhece e que o _Uncle Bob_ se baseou. Estou falando da [_Onion Architecture (arquitetura cebola)_][ONION_PAGE], e acaba deixando de fora alguns outros detalhes que nós entendemos muito importantes.

Ela também não é completa, está faltando essa outra imagem, que na verdade está presente em um cantinho na imagem original.
...
Então o que fizemos foi uní-las ([_Onion Architecture_][ONION_PAGE] + [Hexagonal Architecture][HEXAGONAL_PAGE]), e
obtemos agora uma única imagem que tenta representar ambos os conceitos:
...
1) Existem camadas dispostas concentricamente (como define [_Onion Architecture_][ONION_PAGE])
2) É permitida a comunicação direta das camadas mais externas em direção ao centro
3) A comunicação partindo do centro às extremidades é feita de forma indireta (como define [Hexagonal Architecture][HEXAGONAL_PAGE])
